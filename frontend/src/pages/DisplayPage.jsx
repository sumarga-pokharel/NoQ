...
export default function DisplayPage() {
  ...
  const refresh = useCallback(() => {
    if (!slug) {
      setError("Open a valid office display link.");
      return;
    }

    api(`/public/offices/${slug}/display`)
      .then((result) => {
        setData(result);
        setError("");
      })
    ...
    return (
      <div className="board">
        <AsyncState
          ...
          loading={!error}
          title={
            error ? "Display board unavailable" : "Loading display board"
          }
          message={
            error || "Connecting to the live office queue…"
          }
  ...
  return (
    <div className="board">
      {error && (
        <div className="board__retry" role="alert">
          <span>{error}</span>
      ...
      <div className="board__header">
        <div>
          <div className="board__office-np">NoQ · लाइभ लाइन</div>
      ...
      {!data.counters.length && (
        <AsyncState
          ...
          message="The office will appear here after staff add a counter."
        />
      )}
      ...
      <div className="board__counters">
        {data.counters.map((counter) => (
          <div
            key={counter._id}
            className={`board__counter board__counter--${
              counter.currentTicket ? "default" : "open"
            }`}
          >
            <div className="board__counter-label">
              {counter.name}
            </div>

            {counter.currentTicket ? (
              <div className="board__counter-token">
                {counter.currentTicket.token}
              </div>
            ) : (
              <div className="board__counter-token board__counter-token--empty">
                खाली
              </div>
            )}

            <div className="board__counter-note">
              {counter.currentTicket
                ? "Please come · कृपया आउनुहोस्"
                : "Open"}
            </div>
          </div>
        ))}
      ...
      <div className="board__tiles">
        <div className="board__tile">
          <span className="board__tile-val">
            {data.servingCount}
          </span>
        ...
        <div className="board__tile board__tile--wide">
          ...
          <span className="board__tile-label board__tile-label--light">
            कृपया आफ्नो कागजात तयार राख्नुहोस्
            <br />
            <span>Please keep your documents ready.</span>
          </span>
        </div>
      </div>
      ...
      <div className="board__footer">
        <div>
          <div className="board__next-label">
            क्रमै पालो · Next up
          </div>

          <div className="board__next-list">
            {data.nextUp.map((ticket) => (
